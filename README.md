...
25. [done]可以选择文件夹，每小时会从文件夹内随机选择图片显示（待思考这个特性跟选择一张图片之间怎么兼容。）
    1.  现在做了一版，待优化，因为codex这周的额度用完了。
26. [done]图片缩放最大最小的限制改为根据像素，最小不小于50px，最大宽度不大于屏幕宽度，同时也不大于图片原始宽度的3倍
...
29. [done]增加窗口不会出现在截屏中的功能，截屏时会隐藏窗口，显示后面的内容。

bug：

1. [done]有时候切换编辑状态无法编辑？
