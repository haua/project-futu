//go:build windows

package platform

import (
	"math"
	"unsafe"

	"fyne.io/fyne/v2"
	"fyne.io/fyne/v2/driver"
	"golang.org/x/sys/windows"
)

type winRect struct {
	Left   int32
	Top    int32
	Right  int32
	Bottom int32
}

type winPoint struct {
	X int32
	Y int32
}

const (
	swpNoSize         = 0x0001
	swpNoZOrder       = 0x0004
	swpNoActivate     = 0x0010
	smXVirtualScreen  = 76
	smYVirtualScreen  = 77
	smCXVirtualScreen = 78
	smCYVirtualScreen = 79
)

var (
	user32               = windows.NewLazySystemDLL("user32.dll")
	procGetCursorPos     = user32.NewProc("GetCursorPos")
	procGetWindowRect    = user32.NewProc("GetWindowRect")
	procSetWindowPos     = user32.NewProc("SetWindowPos")
	procGetSystemMetrics = user32.NewProc("GetSystemMetrics")
)

func GetWindowPosition(w fyne.Window) (fyne.Position, bool) {
	nw, ok := w.(driver.NativeWindow)
	if !ok {
		return fyne.Position{}, false
	}

	got := false
	pos := fyne.Position{}
	nw.RunNative(func(context any) {
		winCtx, ok := context.(driver.WindowsWindowContext)
		if !ok {
			return
		}

		hwnd := uintptr(winCtx.HWND)
		if hwnd == 0 {
			return
		}

		var rect winRect
		r1, _, _ := procGetWindowRect.Call(hwnd, uintptr(unsafe.Pointer(&rect)))
		if r1 == 0 {
			return
		}

		pos = fyne.NewPos(float32(rect.Left), float32(rect.Top))
		got = true
	})

	return pos, got
}

func MoveWindowTo(w fyne.Window, x, y float32) bool {
	nw, ok := w.(driver.NativeWindow)
	if !ok {
		return false
	}

	moved := false
	nw.RunNative(func(context any) {
		winCtx, ok := context.(driver.WindowsWindowContext)
		if !ok {
			return
		}
		hwnd := uintptr(winCtx.HWND)
		if hwnd == 0 {
			return
		}

		procSetWindowPos.Call(
			hwnd,
			0,
			uintptr(int32(math.Round(float64(x)))),
			uintptr(int32(math.Round(float64(y)))),
			0,
			0,
			swpNoSize|swpNoZOrder|swpNoActivate,
		)
		moved = true
	})

	return moved
}

func GetCursorPosition() (fyne.Position, bool) {
	var pt winPoint
	r1, _, _ := procGetCursorPos.Call(uintptr(unsafe.Pointer(&pt)))
	if r1 == 0 {
		return fyne.Position{}, false
	}
	return fyne.NewPos(float32(pt.X), float32(pt.Y)), true
}

func IsWindowInVisibleBounds(pos fyne.Position, size fyne.Size) bool {
	left, okL := getSystemMetric(smXVirtualScreen)
	top, okT := getSystemMetric(smYVirtualScreen)
	width, okW := getSystemMetric(smCXVirtualScreen)
	height, okH := getSystemMetric(smCYVirtualScreen)
	if !okL || !okT || !okW || !okH || width <= 0 || height <= 0 {
		return true
	}

	right := left + width
	bottom := top + height

	winLeft := int32(math.Round(float64(pos.X)))
	winTop := int32(math.Round(float64(pos.Y)))
	winRight := int32(math.Round(float64(pos.X + size.Width)))
	winBottom := int32(math.Round(float64(pos.Y + size.Height)))

	return winLeft >= left && winTop >= top && winRight <= right && winBottom <= bottom
}

func getSystemMetric(index int32) (int32, bool) {
	r1, _, _ := procGetSystemMetrics.Call(uintptr(index))
	return int32(r1), true
}
